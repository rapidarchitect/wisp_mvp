...
export function ContributorDomainsPage() {
  ...
  return (
    <Box>
      ...
      {domains !== null && domains.length === 0 && (
        <Typography>No domains assigned yet.</Typography>
      )}
      {domains !== null && (
        <Box sx={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 2 }}>
          {domains.map((d) => (
            <Card key={d.code} data-domain-code={d.code}>
              <CardActionArea onClick={() => navigate(`/domains/${d.code}`)}>
                <CardContent>
                  ...
                  <LinearProgress
                    variant="determinate"
                    value={d.status === "approved" ? 100 : d.status === "in_review" ? 75 : 30}
                    sx={{ mb: 1 }}
                  />
                </CardContent>
              </CardActionArea>
